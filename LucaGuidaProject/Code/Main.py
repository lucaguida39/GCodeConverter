...
#In case data is missing, an error message is shown
    if path_line=='' or (LedChecked.get()==True and (UV_time_line=='' or UV_time_line=='.')) or (ExtraRunChecked.get()==True and (ExtraRun_Distance_Line=='' or ExtraRun_Distance_Line=='.')):
        ...
        ErrorLabel['font'] = ('Helvetica Neue Medium', 14)
        ErrorLabel.place(relx = 0.5, rely = 0.3, anchor = CENTER)
        Ok = Button(Error)
        Ok['text']='Ok'
        Ok['font'] = ('Helvetica Neue Thin', 16)
        Ok['fg'] = '#002a8b'
...
#A dialog window is opened to inform the user that the conversion is taking place
    ...
    InProgressLabel['font'] = ('Helvetica Neue Medium', 14)
...
#After the conversion, the dialog window informs the user about the output
    ...
    Close['text']='Close'
    Close['font'] = ('Helvetica Neue Thin', 16)
    Close['fg'] = '#002a8b'
...
label['text']=string
label['font'] = ('Helvetica Neue Thin', 14)
...
run = Button(root)
run['text']='Convert!'
run['font'] = ('Helvetica Neue Thin', 16)
run['fg'] = '#002a8b'
...
#=====================================================================================
#Folder path textbox
path_textbox = Entry(root, width=int(0.06*W))
path_textbox['font'] = ('Helvetica Neue Thin', 16)
path_textbox.place(x=X2, y=Y1)
PathLabel = Label(root, text = "G-Codes path:")
PathLabel['font'] = ('Helvetica Neue Light', 16)
PathLabel.place(x=X1, y=Y1)

#Button choose path
explore = Button(root)
explore['text']='Choose folder'
explore['font'] = ('Helvetica Neue Thin', 16)
explore['fg'] = '#002a8b'
...
#UV textbox
ledTime_textbox = Entry(root, width=int(0.01*W), validate="key", validatecommand=(root.register(on_validate), '%P'))
ledTime_textbox['state']='disabled'
ledTime_textbox['font'] = ('Helvetica Neue Thin', 16)
...
LedCheckboxLabel['font'] = ('Helvetica Neue Light', 16)
LedCheckboxLabel.place(x=X1, y=Y2)

UVTimeLabel = Label(root, text = "UV time:")
UVTimeLabel['font'] = ('Helvetica Neue Light', 16)
UVTimeLabel.place(x=X3, y=Y2)

SecondsLabel = Label(root, text = "s")
SecondsLabel['font'] = ('Helvetica Neue Thin', 16)
...
#Extra Run textbox
extra_textbox = Entry(root, width=int(0.01*W), validate="key")
extra_textbox['validatecommand'] = (root.register(validate_input), '%P')
extra_textbox['state']='disabled'
extra_textbox['font'] = ('Helvetica Neue Thin', 16)
...
#=====================================================================================
#Line to transform it in an app (to be inserted in terminal)
#=====================================================================================
